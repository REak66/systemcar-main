  ...
  <AppLayout>
    ...
    <div class="max-w-3xl mx-auto">
      ...
      <div class="no-print flex gap-3 mb-4">
        <Link :href="route('invoices.index')" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-200">← {{ t('back') }}</Link>
        <Link :href="route('invoices.edit', invoice.id)" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">{{ t('edit') }}</Link>
        <a :href="route('invoices.download', invoice.id)" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700">⬇ {{ t('download') }}</a>
        <button @click="printInvoice" class="bg-gray-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-800">{{ t('print') }}</button>
      </div>
      ...
      <div id="printable" class="bg-white border border-gray-300 p-8 text-xs font-sans leading-snug">
        ...
        <div class="flex items-start mb-1">
          <div class="flex items-start gap-3">
            <div class="bg-red-600 text-white font-black text-2xl px-2 py-0.5 tracking-widest leading-tight shrink-0" style="margin-top: -5px; -webkit-print-color-adjust: exact; print-color-adjust: exact;">BYD</div>
        ...
        <div class="flex items-center gap-5 border border-gray-400 border-t-0 px-3 py-1.5 mb-3">
          <template v-for="stage in paymentStages" :key="stage.id">
            <label class="flex items-center gap-1 cursor-default">
              <span class="inline-flex items-center justify-center w-4 h-4 border border-gray-600 text-xs leading-none">
        ...
        <table class="w-full border-collapse border border-gray-400 mb-0">
          <thead>
            <tr>
              <th class="border border-gray-400 py-1.5 px-1.5 text-center w-8">No<br><span class="text-gray-500">ល.រ</span></th>
              <th class="border border-gray-400 py-1.5 px-2 text-center">Description<br><span class="text-gray-500">បរិយាយមុខទំនិញ</span></th>
              <th class="border border-gray-400 py-1.5 px-1.5 text-center w-14">Quantity<br><span class="text-gray-500">បរិមាណ</span></th>
              <th class="border border-gray-400 py-1.5 px-1.5 text-center w-28">Unit Price<br><span class="text-gray-500">ថ្លៃឯកតា</span></th>
              <th class="border border-gray-400 py-1.5 px-1.5 text-center w-24">Amount<br><span class="text-gray-500">ថ្លៃទំនិញ</span></th>
            </tr>
          </thead>
          <tbody>
            <!-- Item row -->
            <tr>
              ...
              <td class="border-l border-r border-gray-400 px-2 align-top pt-1.5" style="min-height:130px;">
                <div>Model: {{ invoice.car_model }}</div>
                <div class="mt-0.5">VIN: {{ invoice.notes ?? '' }}</div>
              </td>
              <td class="border-l border-r border-gray-400 px-1.5 text-center align-top pt-1.5">{{ invoice.quantity }}</td>
              <td class="border-l border-r border-gray-400 px-1.5 text-right align-top pt-1.5 whitespace-nowrap">
                <span v-if="Number(invoice.unit_price) > 0">{{ currencySymbol(invoice.currency) }} {{ fmt(invoice.unit_price) }}</span>
              </td>
            ...
            <!-- Spacer rows -->
            <tr style="height:32px;"><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td></tr>
            <tr style="height:32px;"><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td></tr>
            <tr style="height:32px;"><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td></tr>
            <tr style="height:32px;"><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td><td class="border-l border-r border-gray-400"></td></tr>
          </tbody>
          ...
          <tfoot>
            <tr v-if="invoice.with_vat">
              <td colspan="4" class="border border-gray-400 py-1 px-2 text-right">{{ t('vat') }} ({{ invoice.vat_rate }}%)</td>
              <td class="border border-gray-400 py-1 px-2 text-right">{{ currencySymbol(invoice.currency) }} {{ fmt(invoice.vat_amount) }}</td>
            </tr>
            <tr class="border-t border-gray-400">
              <td colspan="4" class="border border-gray-400 py-1.5 px-2 text-right font-semibold">Balance to Pay</td>
              <td class="border border-gray-400 py-1.5 px-2 text-right font-bold">
                {{ currencySymbol(invoice.currency) }} {{ fmt(invoice.grand_total) }}
              </td>
            </tr>
            <tr>
              <td colspan="4" class="border border-gray-400 py-1.5 px-2 text-right font-semibold">ទឹកប្រាក់ដែលត្រូវបង់ (Balance to Pay)</td>
              <td class="border border-gray-400 py-1.5 px-2 text-right font-bold">
                {{ currencySymbol(invoice.currency) }} {{ fmt(invoice.grand_total) }}
              </td>
            </tr>
...
@media print {
  ...
  #printable { border: none !important; box-shadow: none !important; padding: 12mm !important; }
