    ...
    <AppLayout>
        ...
        <div class="max-w-3xl space-y-6">
                ...
                <form @submit.prevent="submitAdd" class="space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input v-model="addForm.name" type="text" :placeholder="'e.g. Branch 1'"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            <p v-if="addForm.errors.name" class="text-red-600 text-xs mt-1">{{ addForm.errors.name }}
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('chat_id') }} *</label>
                            <input v-model="addForm.chat_id" type="text"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            <p v-if="addForm.errors.chat_id" class="text-red-600 text-xs mt-1">{{ addForm.errors.chat_id
                                }}</p>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('bot_token') }} *</label>
                        <input v-model="addForm.bot_token" type="text"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        <p v-if="addForm.errors.bot_token" class="text-red-600 text-xs mt-1">{{ addForm.errors.bot_token
                            }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div v-if="!configs || configs.length === 0" class="px-5 py-8 text-center text-gray-400">{{ t('no_data')
                    }}
                </div>
                ...
                <div v-for="config in configs" :key="config.id" class="border-b border-gray-50 last:border-0">
                    ...
                    <div v-if="editTarget?.id !== config.id" class="px-5 py-4 flex items-center gap-4">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400 font-mono truncate">Token: {{ config.bot_token?.slice(0, 20)
                                }}...
                            </p>
                        </div>
                        <div class="flex gap-2 flex-shrink-0">
                            <button class="text-sm text-blue-600 hover:underline" @click="openEdit(config)">{{ t('edit')
                                }}</button>
                    ...
                    <div v-else class="px-5 py-4 bg-blue-50">
                        <form @submit.prevent="submitEdit" class="space-y-3">
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">{{ t('bot_name')
                                        }}</label>
                                    <input v-model="editForm.name" type="text"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">{{ t('chat_id')
                                        }}</label>
                                    <input v-model="editForm.chat_id" type="text"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">{{ t('bot_token') }}</label>
                                <input v-model="editForm.bot_token" type="text"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div class="flex items-center gap-2">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div v-if="!schedules || schedules.length === 0" class="px-5 py-8 text-center text-gray-400">{{
                    t('no_data') }}
                </div>

                <div v-for="schedule in schedules" :key="schedule.id"
                    class="border-b border-gray-50 last:border-0 px-5 py-4">
                    <form @submit.prevent="saveSchedule(schedule)" class="space-y-3">
                        <!-- Header row: label + toggle switch -->
                        <div class="flex items-center justify-between">
                            <span class="font-medium text-gray-800">{{ t(SCHEDULE_LABELS[schedule.report_type])
                                }}</span>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input v-model="scheduleForms[schedule.id].time" type="time"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                <p v-if="scheduleForms[schedule.id].errors.time" class="text-red-600 text-xs mt-1">{{
                                    scheduleForms[schedule.id].errors.time }}</p>
                            </div>

                            <!-- Day of week (weekly only) -->
                            <div v-if="schedule.report_type === 'weekly'">
                                <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('day_of_week')
                                    }}</label>
                                <select v-model="scheduleForms[schedule.id].day_of_week"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <option v-for="day in DAY_OPTIONS" :key="day.value" :value="day.value">{{ day.label
                                        }}
                                    </option>
                                </select>
                            </div>
                            ...
                            <div v-if="schedule.report_type === 'monthly'">
                                <label class="block text-xs font-medium text-gray-600 mb-1">{{ t('day_of_month')
                                    }}</label>
                                <input v-model.number="scheduleForms[schedule.id].day_of_month" type="number" min="1"
                                    max="31"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                <p v-if="scheduleForms[schedule.id].errors.day_of_month"
                                    class="text-red-600 text-xs mt-1">{{
                                        scheduleForms[schedule.id].errors.day_of_month }}</p>
                            </div>
                        </div>

                        <div class="flex justify-end gap-2">
                            <button type="button" :disabled="sendingNow[schedule.id]"
                                class="bg-gray-100 text-gray-700 px-4 py-1.5 rounded-lg text-sm hover:bg-gray-200 disabled:opacity-50 transition-colors"
